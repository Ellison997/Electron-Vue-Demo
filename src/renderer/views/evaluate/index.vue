  ...
  <div class="evaluate-container">
    <div class="l-table" v-if="remote == 'ON'">
      <div style="text-align: center; font-size: 48px" v-if="startflag != 2">
        当前评价时间：{{
          settingTimeFlag == 1 ? totalTime / 60 + "分钟" : "不定时"
        }}
      </div>
      ...
      <div style="margin-top: 134px" v-if="startflag == 1">
      ...
      <div
        style="margin-top: 100px; text-align: center; padding: 0 130px"
        v-if="startflag == 3"
      >
        <div style="font-size: 64px; margin-bottom: 26px">计算中 ...</div>
        <!-- 计算进度 {{progressNum}} -->
        <!-- <el-progress
          :text-inside="true"
          :stroke-width="26"
      ...
      <div style="margin-top: 10px" v-show="startflag == 2">
        ...
        <div style="text-align: center; line-height: 1.6; font-size: 22px">
          六分钟方均根滑动平均
        </div>
        <div class="l-header">
...
export default {
  ...
  data() {
    return {
      ...
      time: "00:00", // 显示时间
      startflag: 0, // 0 未开始   1. 开始统计   2 统计结束 3 正在计算六分钟
      totalTime: 120 * 60, // 统计时间
      minTotalTime: 7, // 统计时间选择
  ...
  methods: {
    ...
    timesWitch() {
      let that = this;
      let { currentTime, standardTime, totalTime } = that;
      console.log("时间：", currentTime, totalTime, currentTime >= totalTime);
    ...
    // 计算评价时间内的方均根值
    getSpectrumArrIndex(arr, index) {
      let datas = [];
      const sum = (arr) => arr.reduce((acc, val) => acc + val, 0);

      for (const a of arr) {
        // 方
        datas.push(a[index] * a[index]);
      }
      // 均根
      let singleNum = Math.sqrt(sum(datas) / arr.length);

      return singleNum;
    },
    countSixMinutes() {
      let that = this;
      console.log("开始计算六分钟数据：", new Date(), that.startflag);

      let { safetyArr, currentTime, standardTime } = that;
      that.startTime = safetyArr[0][0];

      console.log({ safetyArr, currentTime, standardTime });
      ...
      let pointNum = safetyArr[0][1].length;
      let pointNumArr = [];
      // 频点数组
      for (let index = 0; index < pointNum; index++) {
        let point = sc.fmin + (index * nRbw) / 1000 / 1000 / 2;
        pointNumArr.push(point);
      }
      console.log("频点数组：", pointNumArr);
      const sum = (arr) => arr.reduce((acc, val) => acc + val, 0);

      for (const b of that.bandlist) {
        // 六分钟滑动    = currentTime - standardTime
        for (let index = 0; index <= currentTime - standardTime; index++) {
          let start = new moment(that.startTime)
            .add(index, "second")
            .locale("zh-cn")
            .format("YYYY-MM-DD HH:mm:ss");
          let end = new moment(that.startTime)
            .add(index + standardTime, "second")
            .locale("zh-cn")
            .format("YYYY-MM-DD HH:mm:ss");

          // 计算滑动平均场强
          let intervalRealTimeArr = b.realTimeArr.filter(
            (i) =>
              new Date(start) < new Date(i[1]) &&
              new Date(i[1]) <= new Date(end)
          );
          let mvmArr = intervalRealTimeArr.map((i) => i[0][0]);
          let mvmNum = Number((sum(mvmArr) / mvmArr.length).toFixed(8));

          // 取出滑动计算数据
          let intervalArr = safetyArr
            .filter(
              (i) =>
                new Date(start) < new Date(i[0]) &&
                new Date(i[0]) <= new Date(end)
            )
            .map((i) => i[1]);
          ...
          // 循环所有频点
          for (const pn in pointNumArr) {
            // 取出频段
            if (b.start < pointNumArr[pn] && pointNumArr[pn] <= b.end) {
              let v_m = that.getSpectrumArrIndex(intervalArr, pn);
              if (pointNumArr[pn] < 3000) {
                totCount += (v_m * v_m) / (12 * 12);
              } else {
                totCount += (v_m * v_m) / (0.22 * 0.22 * pointNumArr[pn]);
              }
            }
          }
          // 循环几次push 几次
          b.sixMinutesArr.push([mvmNum, totCount]);
        }
        that.progressNum += (1 / that.bandlist.length) * 100;
        console.log(that.progressNum);
      }
      that.startflag = 2;
      console.log("结束计算六分钟数据：", new Date(), that.startflag);
      that.realChart();
    ...
    disposeListData(arg) {
      ...
      try {
        ...
        let original = [];
        let sArr = arg.split(",");
        let status = sArr.slice(sArr.length - 1);
        let info = sArr.slice(0, 10);
        let listTemp = sArr.slice(10, sArr.length - 1);
        ...
        for (let index = 0; index < listTemp.length; index++) {
          ...
          // 计算频点场强
          // dBmW 转换成 dBμV/m
          let dbuV_m = Number(listTemp[index]) + 107 + AF(point);
          let v_m = Math.pow(10, dbuV_m / 20 - 6); // 场强 v/m
          original.push(v_m);
        }
        that.safetyArr.push([
        ...
        let countmvm = Number(
          (
            Math.sqrt((Number(sum(mWcm2Arr)) / 1000) * (120 * 3.1415926)) * 1000
          ).toFixed(8)
        );
