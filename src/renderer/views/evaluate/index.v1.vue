      ...
      if (currentTime >= totalTime) {
        ...
        for (const b of that.bandlist) {
          ...
          for (let index = 0; index <= currentTime - standardTime; index++) {
            ...
            let startIndex = b.timeArr.findIndex(
              (i) =>
                i ==
            ...
            let endIndex = b.timeArr.findIndex(
              (i) =>
                i ==
                new moment(startTime)
            ...
            if (startIndex == -1) {
              startIndex = b.timeArr.findIndex(
                (i) =>
                  i ==
                  new moment(startTime)
                    .add(index - 1, "second")
            ...
            if (endIndex == -1) {
              endIndex = b.timeArr.findIndex(
                (i) =>
                  i ==
                  new moment(startTime)
                    .add(index + standardTime + 1, "second")
                    .locale("zh-cn")
                    .format("YYYY-MM-DD HH:mm:ss")
              );
            }
            ...
            let mvmArr = b.realTimeArr.slice(startIndex, endIndex).map((i) => i[0]);
            let totArr = b.realTimeArr.slice(startIndex, endIndex).map((i) => i[1]);
            // 六分钟均值数组
            sixMinutesArr.push([
    ...
    disposeListData(arg) {
      ...
      try {
        ...
        for (const b of this.bandlist) {
          ...
          // 实时数据
          b.realTimeArr.push([Number(b.mvm), Number(b.tot)]);
          b.timeArr.push(moment().locale("zh-cn").format("YYYY-MM-DD HH:mm:ss"));
        }
        ...
        // 计算总   ∑   是分 ∑ 的和
        that.bandlist[0].realTimeArr[that.bandlist[0].realTimeArr.length - 1][1] = sum(
          countTotArr
        );
        ...
        // 除于1000 转成w/m2
        that.bandlist[0].realTimeArr[that.bandlist[0].realTimeArr.length - 1][0] = Number(
